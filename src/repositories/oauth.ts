...
export class AuthRepo extends BaseRepo {
	...
	public async RegisterMSSQL(data: RegisterInput): Promise<ResponseResult> {
		try {
			const validateResult = await this.validateInput(data);
			if (validateResult !== true) {
				return validateResult;
			...
			await this.mssqlDb.save(newUser);
	...
	public async LoginMSSQL(data: LoginInput): Promise<ResponseResult> {
		try {
	...
	private async validateInput(data: RegisterInput): Promise<any> {
		...
		const emailExist = await this.mssqlDb.findOne({email: data.email});
		if (emailExist) {
			return this.Failure(ErrorCodes.Validate.EMAIL_EXIST);
		}
		const usernameExist = await this.mssqlDb.findOne({username: data.username});
		if (usernameExist) {
